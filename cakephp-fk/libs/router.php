	...
	function url($url = null, $full = false) {
		...
		if (is_array($url)) {
			...
			if (((strpos($url, '://')) || (strpos($url, 'javascript:') === 0) || (strpos($url, 'mailto:') === 0)) || (!strncmp($url, '#', 1))) {
                /** FLIPKEY EDIT - JL */
                return Router::enforceTrailingSlash($url);
                //return $url;
                /** END FLIPKEY EDIT */
			}
			if (empty($url)) {
        ...
        $output_extension = Router::enforceTrailingSlash($output.$extension);

        return $output_extension . $_this->queryString($q, array(), $escape) . $frag;
 ...
 */
    ...
    /**
     * Enforce trailing slashes for url strings.
     * @param string $url
     * @return string $url
     */
    public static function enforceTrailingSlash($url) {
        // Find position of ?
        if(!empty($url)) {

            // Dont enforce trailing slashes on javascript
            if(stripos($url, 'javascript:') !== false) {
                return $url;
            }

            // Dont enforce trailing slashes on extensions ie: .css, .js, .jpg
            if(preg_match('/^.+\..{1,5}$/', $url)) {
                return $url;
            }
            ...
            // If no url is given, return the query string/fragment untouched
            if($qpos === 0 || $fpos === 0) {
                return $url;
            }

            if($qpos === false && $fpos === false) {
                // No query string or fragment detected, add trailing slash
                if(substr($url, -1) !== '/') {
                    $url .= '/';
                }
            } elseif($qpos) {
                if($fpos !== false && $fpos < $qpos) {
                    // Situation where fragment contains a question mark
                    $url = self::_insertChar($url, $fpos, '/');
                } else {
                    // Query string detected, add trailing slash
                    $url = self::_insertChar($url, $qpos, '/');
                }
            } elseif($qpos === false && $fpos) {
                // No query string, but a fragment, add trailing slash
                $url = self::_insertChar($url, $fpos, '/');
            } else {
                // Query string and fragment, add slash before query string
                $url = self::_insertChar($url, $qpos, '/');
            }
    ...
    /**
     * Helper function to insert a $char into $string at $pos if it doesnt exist.
     * @param string $string
     * @param int $pos
     * @param string $insert
     * @return string $string with inserted char if necessary
     */
    protected static function _insertChar($string, $pos, $char) {
        if(substr($string, $pos-1, 1) !== $char) {

		...
		if ($boot) {
			...
			if (Configure::read('Cache.disable') !== true) {
				...
				if (Cache::config('_cake_core_') === false) {
				    if (!empty($cache['settings'])) {
    				    $core_arr = array_merge($cache['settings'], array(
					...
					Cache::config('_cake_core_', $core_arr);
				}

				if (Cache::config('_cake_model_') === false) {
				    if (!empty($cache['settings'])) {
    				    $model_arr = array_merge($cache['settings'], array(
					...
					Cache::config('_cake_model_', $model_arr);
				}

...
App::import('Vendor','FlipkeyLogger', array('file' => 'FlipkeyLogger.php'));
 ...
 */
    ...
    protected function _logDatabaseError($msg) {
        FlipkeyLogger::getLogger('database');
        FlipkeyLogger::error("$this: $msg");
    }

    public function _logQueryError($sql) {
        $debug = '';
        $trace = debug_backtrace();
        array_shift($trace);

        foreach ($trace as $call) {
            $debug .= $call['function'].'() - '.$call['file'].' line '.$call['line']."\n";
        }

        $this->_logDatabaseError($this->lastError() . " in query '" . $sql . "'" . ". trace:\n$debug");
    }
 ...
 */
	function fetchAll($sql, $cache = true, $modelName = null) {
		...
		if ($this->execute($sql)) {
			...
			if ($cache) {
                // FLIPKEY_EDIT - JDL - Don't need to trim or strtolower here, just use stripos
 ...
 */
    //@param $sorted - never used by parent that calls this
	function showLog($sorted = false)
    {
		$log = $this->_queriesLog;
        $log = array_filter($log, 'mute_describes');
        
        //Write sql to Log
		if (PHP_SAPI == 'cli' || Configure::read('FlipKey.sql_log_style') == 'file')
        {
            $this->_show_log_file($log);
        }
        //Some views override this (like where there is no pretty html css/js)
        else if (Configure::read('FlipKey.sql_log_style') == 'basic_html')
        {
            $o = $this->_show_log_basic_html($log);
            echo $o;
        }
        //assime 'pretty html'
        else
        {
            $o = $this->_show_log_pretty_html($log);
            echo $o;
        }
	}
    
    function _show_log_file($log)
    {
        $o = $_SERVER['REQUEST_URI']."\n";
    	foreach ($log as $k => $i) {
			$o .= (($k + 1) . ". {$i['query']} {$i['error']}\n");
		}
        
        $this->log($o, "queries");
    }
    
    function _show_log_pretty_html($log)
    {
        $toggle_text = '<b style="margin-right: 10px">'.$this->configKeyName.'</b> Queries: '.$this->_queriesCnt.' Exec Time: '.$this->_queriesTime.' ms';
        $o = ('<div class="debug_box"><a class="cakeSqlLog-toggle toggle" onclick="$(this).next().toggle(); return false;">'.$toggle_text.'</a>');
        $o .=  ("<table class=\"cake-sql-log\" style=\"display: none\" id=\"cakeSqlLog_" . $this->configKeyName . "\" cellspacing=\"0\" border = \"0\">");
        $o .=  ("<thead>\n<tr><th style='width: 30px'>Nr</th><th style='width: 100%'>Query</th><th>Error</th><th>Affected</th><th style='width: 30px'>Num. rows</th><th style='width: 30px'>Took (ms)</th></tr>\n</thead>\n<tbody>\n");
    
        foreach ($log as $k => $i) {
            $lines = count(explode("\n", h($i['query'])));
            $o .=  ("<tr><td>" . ($k + 1) . "</td><td><div onclick=\"$(this).next().setStyle({visibility: 'visible', height: ($(this).getHeight() + 20) + 'px'}); $(this).hide();\">".h($i['query'])."</div><textarea rows=\"$lines\">" . h($i['query']) . "</textarea></td><td>{$i['error']}</td><td style = \"text-align: right\">{$i['affected']}</td><td style = \"text-align: right\">{$i['numRows']}</td><td style = \"text-align: right\">{$i['took']}</td></tr>\n");
        }
        $o .= ("</tbody></table></div>\n");
        return $o;    
    }
    
    function _show_log_basic_html($log)
    {
        $o =  "<table class=\"cake-sql-log\" id=\"cakeSqlLog_" . $this->configKeyName . "\" cellspacing=\"0\" border = \"0\">";
        $o .=  "<thead>\n<tr><th style='width: 30px'>Nr</th><th style='width: 100%'>Query</th><th>Error</th><th>Affected</th><th style='width: 30px'>Num. rows</th><th style='width: 30px'>Took (ms)</th></tr>\n</thead>\n<tbody>\n";
    
        foreach ($log as $k => $i) {
            $lines = count(explode("\n", h($i['query'])));
            $o .=  "<tr><td>" . ($k + 1) . "</td><td>". h($i['query']) . "</td><td>{$i['error']}</td><td style = \"text-align: right\">{$i['affected']}</td><td style = \"text-align: right\">{$i['numRows']}</td><td style = \"text-align: right\">{$i['took']}</td></tr>\n";
        }
        $o .= "</tbody></table>\n";
        return $o;
    }
...
//Prevent debug mode for describing each table in the logs
function mute_describes($line)
{
    if (substr($line['query'], 0, 8) == 'DESCRIBE') return false;
    return true;
}

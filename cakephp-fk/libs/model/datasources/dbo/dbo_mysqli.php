 ...
 */
	function connect() {
        ...
        /**
         * Customized to ensure that connection timeouts get logged by the flipkey logger shutdown function.
         *
         * The original line...
         * 
         *   $this->connection = mysqli_connect($config['host'], $config['login'], $config['password'], $config['database'], $config['port'], $config['socket']);
         *   
         * ...seemed to not always log this message because it appeared the shutdown function wasn't always called.  Not sure why.
         */
        $this->connection = mysqli_init();
        $this->connection->options(MYSQLI_OPT_CONNECT_TIMEOUT, $this->_getConnectTimeout());
        ...
        if(!$this->isConnected()) {
            $this->_logDatabaseError(mysqli_connect_errno() . ": " .  mysqli_connect_error());
        }
    ...
    public function setWaitTimeout($timeout_secs) {
        $this->execute("SET session wait_timeout=" . (int)$timeout_secs);
    }

    public function resetWaitTimeout() {
        $this->setWaitTimeout($this->_getWaitTimeout());
    }
    ...
    protected function _getConnectTimeout() {
        return $this->_getConfiguredTimeout('Db.connectTimeout');
    }

    protected function _getWaitTimeout() {
        return $this->_getConfiguredTimeout('Db.waitTimeout');
    }

    private function _getConfiguredTimeout($key, $fallback=30) {
        // should return timeout or null on error
        $timeout = Configure::read($key);

        if($timeout > 0) {
            return $timeout;
        }

        // invalid timeout returned from config so use our fallback
        return $fallback;
    }
 ...
 */
	function _execute($sql) {
        // FlipKey edit: JCS 9-17-2012: added to better handle failure to connect issues we are seeing in
        // production

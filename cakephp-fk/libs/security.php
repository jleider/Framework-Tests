 ...
 */
	 ...
	 * @param string $text Encrypted string to decrypt, normal string to encrypt
	 * @param string $key Key to use
	 * @param boolean $cipher_speedu if true will run our new faster cipher code if false then run the built in cakePHP cipher code
	 * @return string Encrypted/Decrypted string
	 * @access public
	 * @static
	 */
    function cipher($text, $key = '', $cipher_speedy = true) {
        if (empty($key)) {
            trigger_error(__('You cannot use an empty key for Security::cipher()', true), E_USER_WARNING);
            return '';
        }
        if (!$cipher_speedy )
            Security::cipher_cake($text, $key);

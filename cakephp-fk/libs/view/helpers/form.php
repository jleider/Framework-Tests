...
class FormHelper extends AppHelper {
	/**** FLIPKEY EDIT - MODIFIED BY FLIPKEY PART 1 **
	 * get instance for HtmlHelper  **/
	function __construct()
	{
		App::Import('Helper', 'Html');
		$this->Html = new HtmlHelper();
	}
	/**** END FLIPKEY EDIT - MODIFIED BY FLIPKEY PART 1 ****/
/**
 ...
 */
	function create($model = null, $options = array()) {
        ...
        if(is_object($this->data) || is_object($model)) {
            App::import('Vendor','FlipkeyLogger');
            FlipkeyLogger::getLogger('form_helper');
            FlipkeyLogger::error('BEGIN ERROR');
            FlipkeyLogger::error('Model: '.var_export($model, true));
            FlipkeyLogger::error('Options: '.var_export($options, true));
            FlipkeyLogger::error('this->data: '.print_r($this->data, true));
            FlipkeyLogger::error(FlipkeyUtil::getDebugBacktrace());
            FlipkeyLogger::error('END ERROR');
        }
        /** END FLIPKEY EDIT */
 ...
 * @param bool $always_return_fix - LB - We need a div for making forms space correctly in the case that there is not an error as well.
 * @return string If there are errors this method returns an error message, otherwise null.
 * @access public
 */
	function error($field, $text = null, $options = array(), $always_return_div = true, $show_icon = true) {
		$defaults = array('wrap' => true, 'class' => 'error-message lv_error_box', 'id' => $this->model().Inflector::camelize($field).'_errors', 'escape' => false);
		...
		if ($error = $this->tagIsInvalid()) {
			if (is_array($error)) {
				...
				if (isset($error[$field])) {
                    ...
                    if ($always_return_div)
                    {
                		$tag = is_string($options['wrap']) ? $options['wrap'] : 'div';
                        unset($options['wrap']);
                        return $this->Html->tag($tag, "", $options);
                    }
					else return null;
				}
			}
			...
			if ($options['wrap']) {
                ...
                if ($show_icon) $error_icon = $this->Html->image('/img/error_icon.gif');
                else $error_icon = "";
				return $this->Html->tag($tag, $error_icon.$error, $options);
			} else {
				return $error;
			}
		} else {
 ...
 */
    ...
    /**
     * LB - Set an array of php arrays that represent LiveValidation object, that will be json encoded and added as the lv attribute of inputs created.
     **/
    function setLiveValidation($validations)
    {
        $this->liveValidation = $validations;
    }
 ...
 */
	function input($fieldName, $options = array()) {
		...
		// FLIPKEY EDIT - Change defaults to include label and div
		$defaults = array('before' => null, 'between' => null, 'after' => null, 'label' => '', 'div' => '');
		// END FLIPKEY EDIT - Change defaults to include label and div
        ...
        // BEGIN FlipKey Edit, add LiveValidation arrays if we have them //
        if ($this->liveValidation[$fieldName]) $options['lv'] = $this->liveValidation[$fieldName];
		...
		if ($type != 'hidden') {
			$out .= $after;
			if ($error !== false && isset($this->liveValidation)) {

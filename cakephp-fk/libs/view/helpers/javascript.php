 ...
 */
	function link($url, $inline = true) {
		...
		if (strpos($url, '://') === false) {
            ...
            $payment_server = (Configure::read('Flipkey.payment_server') == "true");

			if (Configure::read('Asset.filter.js') && !$payment_server) {
				$url = str_replace(JS_URL, 'cjs/', $url);
			}else if ($payment_server){
                $url = str_replace(JS_URL, 'payments-js/', $url);
            }

            if (Configure::read('Flipkey.version')) {                
                $version_css = "?ver=" . Configure::read('Flipkey.version');
				$url .= $version_css;
			}
		}
